...
def get_spfh(point_cloud, nearest_idx, keypoint_id, radius, B): 
...
def describe(point_cloud, nearest_idx, keypoint_id, radius, B):  
    ##step5 寻找每个keypoint 的nearest points
    points = np.asarray(point_cloud)
    keypoint = np.asarray(point_cloud)[keypoint_id]
    #remove query point 去除关键点  :
...
if __name__ == '__main__':
    point_cloud = np.genfromtxt(r"/Users/seaside/Desktop/【第02部分 三维点云专题】三位建模感知与点云视频教程 5套/03.2020年3D感知技术培训视频教程（附代码、讲义、参考资料）49课/00.配套资料（代码、讲义、作业、参考文献）/00.配套资料（代码、讲义、作业、参考文献）/07.人体、物体识别与点云配准/CH7-HW/feature/airplane_0001.txt", delimiter=",")
    ...
    ##step4 求解每个关键点的 spfh
    B = 5  # 每个 直方图 bin的个数
    nearest_idx = search_tree.query_radius(point_cloud_raw, radius)
