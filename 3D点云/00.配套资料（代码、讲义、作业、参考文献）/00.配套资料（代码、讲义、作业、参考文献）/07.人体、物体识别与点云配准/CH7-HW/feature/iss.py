...
# matplotlib显示点云函数
def show(point_cloud,feature_point):
...
def compute_cov_eigval(point_cloud):
    x = np.array(point_cloud[:,0])
    y = np.array(point_cloud[:,1])
    z = np.array(point_cloud[:,2])
...
if __name__ == '__main__':
    ...
    print(feature_point)
    show(point_cloud,feature_point)