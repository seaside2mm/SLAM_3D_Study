...
MotionData IMU::MotionModel(double t) {
    ...
    Eigen::Vector3d dp(
            -K * ellipse_x * sin(K * t),
            K * ellipse_y * cos(K * t), z * K1 * K * cos(K1 * K * t));              // position导数　in world frame
    double K2 = K * K;
    Eigen::Vector3d ddp(
            -K2 * ellipse_x * cos(K * t),
            -K2 * ellipse_y * sin(K * t), -z * K1 * K1 * K2 * sin(K1 * K * t));     // position二阶导数

    // Rotation
    double k_roll = 0.1;
    double k_pitch = 0.2;
    Eigen::Vector3d eulerAngles(
            k_roll * cos(t), k_pitch * sin(t), K * t);   // roll ~ [-0.2, 0.2], pitch ~ [-0.3, 0.3], yaw ~ [0,2pi]
...
//    Eigen::Vector3d eulerAnglesRates(0.,0. , K);      // euler angles 的导数
    ...
    Eigen::Vector3d imu_gyro =
            eulerRates2bodyRates(eulerAngles) * eulerAnglesRates;   //  euler rates trans to body gyro
...
void IMU::testImu(std::string src, std::string dist) {
    ...
    bool use_media_value_method = false;
    if (!use_media_value_method) {
